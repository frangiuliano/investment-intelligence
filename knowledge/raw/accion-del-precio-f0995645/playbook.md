...
- Treat S/R as **zones**, not exact prints: wicks often *test* without a true break; one close through a level can still fail (false break) — prefer follow-through over wick noise; closes-only views can clarify zone edges.
- After an alleged break, a short pullback into former resistance (now support) may confirm direction — still not certainty.
- Classic Fib retracements (0.236–0.764) / expansions are **crowd-expectation** zones (0.236 often weakest); swing high/low placement is subjective — never use Fib alone.
- Reversal candlesticks (doji after a long body; hammer vs hanging man; inverted hammer vs shooting star) need a **prior trend** and a **confirming close** past the reference open — same silhouette ≠ same meaning without context.
- Marubozu (no shadows) = strong one-sided session; spinning tops = indecision — still require structure/context.
- Pivot points (prior-session derived): state trend first; more useful in ranges; news/volatility can blow through outer pivots — prefer confluence with structure / candles / MAs / oscillators.
- MACD (MA-of-MAs + histogram) can flag trend change but **lags** — do not treat crosses as early certainty without structure.
- There is no magic indicator stack; prefer a few agreeing tools and know how each reacts to price.
- **Price action** lens: prefer OHLC structure (decision vs indecision bodies, rejection wicks at S/R, broken support → future resistance) over indicator-only stories; large close beyond a zone with little opposing wick strengthens a break narrative vs soft wick pierce.
- Compression patterns (triangles / narrowing S/R): wait for a side to break with follow-through — do not lean on the apex alone; shrinking candles into a base then decision thrust can mark exhaustion → reversal *narratives*, still needing confirmation.
- Engulfing / covering prior body and three-soldier/crow-style runs: require prior trend; larger/separated bodies are stronger cues; oversized decision bars often invite a small next-bar correction — not certainty.
- Weinstein-style break context: clear thrust **with volume** / force through S/R and MAs beats soft pierces; state whether the break looks “soft” (hidden opposing interest) vs “forceful.”
- Choppy MACD/stochastic (small messy peaks) → wait for cleaner waves/structure; divergence (price vs oscillator) is correction caution, not an automatic reverse.
- High-impact macro calendar prints can invalidate short-horizon technical theses in seconds — note timing when relevant.
- Keep written rules (when lean / when skip / risk bound); reject emotional impulse overrides and miracle win-rate claims — expectancy over long samples.
...
| Headline only names Fib retracement / pivot / doji / hammer / MACD | low | Pattern/indicator jargon ≠ catalyst |
| Fib or pivot used as sole reason to upgrade an alert | low | Educational lens only |
| Wick pierce or single close treated as definitive S/R break | low | False-break risk; need follow-through |
| Certainty that a Fib level or candle silhouette “must” reverse | low | Process smell |
| Headline only names price action / engulfing / triangle / “vela de decisión” | low | Pattern jargon ≠ catalyst |
| Soft wick pierce treated as confirmed S/R break | low | Need close + follow-through |
| Miracle strategy / 90% win-rate framing | low | Process smell; ignore for alerts |
| Martingale / doubling after losses as “risk control” | low | Anti-method; do not echo |
...
- Wick pierce or one close through S/R treated as a definitive break without follow-through.
- Fib or pivot used alone; swing anchors redrawn post-hoc so levels never fail.
- Reversal candle cited without prior trend or without confirming close.
- Range tools (Bollinger bounce, pivots R1–S1) applied in a clear strong trend without stating regime.
- LTF thesis that ignores opposing HTF structure (e.g. “bullish” LTF at HTF channel top).
- MACD (or similar lagging) cross treated as early certainty without structure.
- Neutral/doji in a no-trend regime treated as directional.
- Triangle/compression apex traded without a break and follow-through.
- Choppy indicator stack forced into a thesis (“revuelto” ignored).
- Soft pierce without volume/force treated as a Weinstein-style breakout.
...
- Map “buy at Fib / enter at pivot / stop at swing / pip target” language into product orders.
- Invent pip counts, lot sizes, or broker-setup steps as research method.
- Promote candlestick or indicator names to `event_type`.
- Map “cruce estocástico / MM mágica / pips / binarias / martingala” into product orders or sizing.
- Echo miracle win-rate or impulse-override language as method.
...
- operator: `sources/babypips- Tus primeros pasos en Forex.pdf` (docId `babypips-tus-primeros-pasos-en-forex-6d5491fb`; Fin-ranked `technical_analysis` via `filter-themes.json` v1.1.0 — FX intro mapped to equity research lens)
- operator: `sources/ACCION DEL PRECIO.pdf` (docId `accion-del-precio-f0995645`; Fin-ranked `technical_analysis` via `filter-themes.json` v1.1.0 — price-action deck; discard binary/martingale recipes)
